...
bool NetworkMessage::ReadFromSocket(SOCKET socket)
{
	...
	if(m_encryptionEnabled){
		if(!m_keyset){
			qDebug() << "Failure: [NetworkMessage::ReadFromSocket]. Key not set";
			//std::cout << "Failure: [NetworkMessage::ReadFromSocket]. Key not set" << std::endl;
			return false;
		}
		if((m_MsgSize - 2) % 8 != 0){
			qDebug() << "Failure: [NetworkMessage::ReadFromSocket]. Not valid encrypted message size";
		...
		int tmp = GetU16();
		if(tmp > m_MsgSize - 4){
			qDebug() << "Failure: [NetworkMessage::ReadFromSocket]. Not valid unencrypted message size";
...
void NetworkMessage::XTEA_encrypt()
{
	...
	while(read_pos < (unsigned)m_MsgSize/4){
...
bool NetworkMessage::RSA_encrypt()
{
	...
	if(newPos < 0)
	{
		qDebug() << "Warning: [NetworkMessage::RSA_decrypt()]. Not valid packet size";
		return false;
	}

	if(!m_RSA)
	{
		qDebug() << "Warning: [NetworkMessage::RSA_decrypt()]. m_RSA no set";
		return false;
	}
